...
const MediaPage = () => {
    ...
    return <div className="z-0 lg:flex">
        ...
        <div className="lg:p-16 md:p-10 sm:p-6 p-3 flex flex-wrap gap-10 h-fit">
            {urlList.length === 0 ? <p className="text-lg text-red-800">Your media will be here</p> : <>
                {urlList.map((item, index) => (
                    ...
                    >
                        ...
                        />
                        <div className="rounded-lg h-full w-full absolute top-0 left-0 z-10 flex items-end justify-end transition hover:opacity-100 lg:opacity-0">
                            <div className="flex m-1 md:m-3 bg-gray-200 rounded-sm">
                                <p className="w-20 truncate">{item.url.split("/").pop().split("?")[0]}</p>
                                ...
                                <button onClick={() => { handleDeleteImage(item.url) }} className="hover:cursor-pointer p-1"><Image src={"/icons/deleteIcon.png"} alt="delete icons" width={20} height={20} /></button>
                            </div>
                        </div>
                    </div>
                ))}</>}
